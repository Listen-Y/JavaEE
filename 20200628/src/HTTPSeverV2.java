...
public class HTTPSeverV2 {
    ...
    public void process(Socket clientSocket) {
        try {
            // 1. 读取并解析请求
            HTTPRepuestHealper request = HTTPRepuestHealper.build(clientSocket.getInputStream());
            System.out.println("request: " + request);
            HTTPRespondHealper response = HTTPRespondHealper.bulid(clientSocket.getOutputStream());
            ...
            if (request.getUrl().startsWith("/hello")) {
                ...
                response.setStatus(200);
                response.setMessage("OK");
                response.setBody("<h1> result = " + result + "</h1>");
    ...
    public static void main(String[] args) throws IOException {
        HTTPSeverV2 server = new HTTPSeverV2(9090);
        server.start();
    }
}